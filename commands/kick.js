...
exports.run = async(message, client, args) => {
    ...
    let username = args[0];
    if(!username) {
        return message.channel.send(client.embedMaker(message.author, "No Username Supplied", "You didn't supply a username for me to kick from the game"));
    }

    let reason = args.splice(1).join(" ");
    if(!reason) {
        return message.channel.send(client.embedMaker(message.author, "No Reason Supplied", "You didn't supply a reason for me to kick this user with"));
    }
