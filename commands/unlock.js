...
exports.run = async(message, client, args) => {
    ...
    if(unlockCoolDowns.has(message.author.id)) {
    ...
    let type = args[0];
    if(!type) {
        return message.channel.send(client.embedMaker(message.author, "No Type Supplied", `You didn't supply the type of unlock for me to preform`));
    }

    if(type.toLowerCase() !== "global" && type.toLowerCase() !== "jobid") {
        return message.channel.send(client.embedMaker(message.author, "Invalid Type Supplied", "The type of unlock that you supplied isn't valid"));
    }
    ...
    if(type.toLowerCase() === "global") {
        ...
        unlockCoolDowns.add(message.author.id);
        ...
        let jobID = args[1];
        if(!jobID) {
            return message.channel.send(client.embedMaker(message.author, "No Job ID Supplied", `You didn't supply a job id for the server that you want me to unlock`));
        }
...
exports.help = async() => {
    let name = "**unlock global | unlock jobid <job id>**";
    let description = "Unlocks servers with the given settings";
