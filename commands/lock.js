...
exports.run = async(message, client, args) => {
    ...
    if(lockCoolDowns.has(message.author.id)) {
    ...
    let type = args[0];
    if(!type) {
        return message.channel.send(client.embedMaker(message.author, "No Type Supplied", `You didn't supply the type of lockdown for me to preform`));
    }

    if(type.toLowerCase() !== "global" && type.toLowerCase() !== "jobid") {
        return message.channel.send(client.embedMaker(message.author, "Invalid Type Supplied", "The type of lockdown that you supplied isn't valid"));
    }
    ...
    if(type.toLowerCase() === "global") {
        ...
        if(!reason) {
            return message.channel.send(client.embedMaker(message.author, "No Reason Supplied", `You didn't supply a reason for me to lock all the servers`));
        }
        ...
        let jobID = args[1];
        if(!jobID) {
            return message.channel.send(client.embedMaker(message.author, "No Job ID Supplied", `You didn't supply a job id for the server that you want me to lock`));
        }

        let reason = args.splice(2).join(" ");
        if(!reason) {
            return message.channel.send(client.embedMaker(message.author, "No Reason Supplied", `You didn't supply a reason for me to lock the server`));
        }
...
exports.help = async() => {
    let name = "**lock global <reason> | lock jobid <job id> <reason>**";
    let description = "Locks servers with the given settings";
