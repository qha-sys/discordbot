...
exports.run = async(message, client, args) => {
    ...
    let username = args[0];
    if(!username) {
        return message.channel.send(client.embedMaker(message.author, "No Username Supplied", `You didn't supply a username for me to check ban status on`))
    }
...
exports.help = async() => {
    let name = "**checkban <username>**";
    let description = "Checks the ban status of the supplied username";
